% !TEX encoding = UTF-8 Unicode
\documentclass[a4paper,12pt,italian,oneside]{book}   	% use "amsart" instead of "article" for AMSLaTeX format
%\usepackage[parfill]{parskip}    		% Activate to begin paragraphs with an empty line rather than an indent
\usepackage{graphicx}				% Use pdf, png, jpg, or epsß with pdflatex; use eps in DVI mode
								% TeX will automatically convert eps --> pdf in pdflatex		
\usepackage{amssymb}
\usepackage[italian]{babel}			% Traduce in italiano tutte le parti
\usepackage[utf8]{inputenc}			% Permette l'inserimento di caratteri in UTF-8

\usepackage{hyperref}				% Per rendere cliccabile l'indice
\hypersetup{colorlinks=true,linkcolor=blue}

%\setcounter{tocdepth}{1}			% Setta la profondità dell'indice

\title{Titolo}
\author{Andrea Peretti}
%\date{}							% Activate to display a given date or no date


\begin{document}

\maketitle
\tableofcontents

%%%%%%%%
\input{capitolo1/cap1}

%%%%%%%%
\input{capitolo2/cap2}

%%%%%%%%
\input{capitolo3/cap3}

%%%%%%%%
\input{capitolo4/cap4}


\end{document} 